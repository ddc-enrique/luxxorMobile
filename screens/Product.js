 ...
 const [detailsOn, setDetailsOn] = useState(false);
  ...
  useEffect(()=>{
    if(props.products.length===0){
      props.getProduct(props.route.params.id)
      .then((res)=>{
        if(!res.data.success){
          console.log("Disculpe,tenemos problemas tecnicos,vuelva en unos minutos.")
        }else{
     ...
     .catch(error=>{
      ...
      setProduct(props.products.find(product=> product._id===props.route.params.id))
    }
  },[])
